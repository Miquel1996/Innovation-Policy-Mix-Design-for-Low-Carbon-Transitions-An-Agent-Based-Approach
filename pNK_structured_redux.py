...
class Statistician:
    def __init__(self, results_list, params):
        self.results_list = results_list if isinstance(results_list, list) else [results_list]
        self.params = params
    ...
    def compute_metrics(self, histories):
        ...
        for var in ['market_share','profit','impact','productivity','sustainability','markup','price','belief','units','fitness','profit_rate']:
    ...
    def compute_mc_metrics(self, ci=95):
        ...
        var_list = ['market_share','units','profit','impact','productivity','sustainability','markup','price','profit_rate','belief','fitness']

        # Calculate median and CI across runs
        for var in var_list:
    ...
    def plot_mc_metrics(self, mc_metrics):
        var_list = mc_metrics.keys()
        for var in var_list:
            fig, axes = plt.subplots(1, 3, figsize=(18, 4), sharex=True)
            fig.suptitle(f"{var.capitalize()} Dynamics (Monte Carlo Median & CI)")

            for i, key in enumerate(['mv','std','skew']):
                median = mc_metrics[var][key]['median']
                low = mc_metrics[var][key]['low']
                high = mc_metrics[var][key]['high']

                axes[i].plot(median, color='black', label='Median')
                axes[i].fill_between(range(len(median)), low, high, color='gray', alpha=0.3, label='95% CI')
                axes[i].grid(True)
                axes[i].legend()
...
class Paralleler:
    @staticmethod
    def run_mc_fixed(params, n_runs, n_processes, overlay_debug=False, auto_plot=True, debug_print=True):
        seeds = [(np.random.randint(0, 1e6), np.random.randint(0, 1e6)) for _ in range(n_runs)]
        print("DEBUG: Seeds for MC runs:", seeds[:5])
        ...
        if debug_print:
            for run_idx, run in enumerate(results):
                ...
                print("Market share HHI:", [np.sum(run['market_share'][:, t]**2) for t in range(15)])
        ...
        # Stats integration: Single run vs MC
        if n_runs > 1:  
            stats = Statistician(results, params)
            metrics = stats.compute_mc_metrics(ci=95)
            if auto_plot:
                stats.plot_mc_metrics(metrics)
        else:
            stats = Statistician(results[0], params)
            metrics = stats.compute_metrics(results[0])
            if auto_plot:
...
if __name__ == "__main__":
    debug_params = {
        ...
        'w_final': 0, 'w_init': 0.01, 'Tstart_w': 1, 'Tend_w': 1, 'chi_w': 0.1,
        'price_rule': 0, 'procyclical': 0, 'free_research': False
    }

    results, stats, metrics = Paralleler.run_mc_fixed(
        debug_params, n_runs=1, n_processes=600, overlay_debug=True, auto_plot=True
    )

    print(stats.summary_table(metrics))
